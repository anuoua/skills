...
|----------|----------------|--------------|
| participant | `collect` → *"raise your hand or skip"* | `raise` |
| participant | `your_turn` → *"your turn to speak"* | `send` |
...
| host | `round_done` → *"start next round or terminate"* | `collect` (next) or `kill` |
